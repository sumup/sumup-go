...
func TestEventHandler_parseVerified(t *testing.T) {
	t.Parallel()

	client := &Client{}
	handler := NewEventsHandler(WithClient(client))
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := handler.parseVerified(bytes.NewReader([]byte(tt.body)))
			if err != nil {
			...
			assertEventNotificationCommonFields(t, got, client)
		})
	}
}

func TestEventHandler_parseVerified_errors(t *testing.T) {
	t.Parallel()
	...
	t.Run("invalid json", func(t *testing.T) {
		...
		_, err := handler.parseVerified(bytes.NewReader([]byte("{")))
		if err == nil {
	...
	t.Run("invalid event payload", func(t *testing.T) {
		...
		_, err := handler.parseVerified(bytes.NewReader(payload))
		if err == nil {
			t.Fatal("parseVerified() error = nil, want error")
		}
	})
...
func TestEventNotification_FetchObject(t *testing.T) {
	t.Parallel()
	...
	t.Run("fetches referenced checkout", func(t *testing.T) {
		...
		event := &EventNotification[Checkout]{
			Object: Object{
...
func assertEventNotificationCommonFields(t *testing.T, event eventNotification, client *Client) {
	t.Helper()
	...
	case *CheckoutCreatedEvent:
		assertEventNotificationFields(t, &evt.EventNotification, client, "evt_123", "checkout.created", wantCreatedAt)
	case *CheckoutProcessedEvent:
		assertEventNotificationFields(t, &evt.EventNotification, client, "evt_123", "checkout.processed", wantCreatedAt)
	case *CheckoutFailedEvent:
		assertEventNotificationFields(t, &evt.EventNotification, client, "evt_123", "checkout.failed", wantCreatedAt)
	case *CheckoutTerminatedEvent:
		assertEventNotificationFields(t, &evt.EventNotification, client, "evt_123", "checkout.terminated", wantCreatedAt)
	case *MemberCreatedEvent:
		assertEventNotificationFields(t, &evt.EventNotification, client, "evt_123", "member.created", wantCreatedAt)
	case *MemberRemovedEvent:
		assertEventNotificationFields(t, &evt.EventNotification, client, "evt_123", "member.removed", wantCreatedAt)
	case *UnknownEventNotification:
		eventNotification := (*EventNotification[any])(evt)
		assertEventNotificationFields(t, eventNotification, client, "evt_123", "something.else", wantCreatedAt)
	default:
		t.Fatalf("unexpected event type %T", event)
	}
}

func assertEventNotificationFields[T any](t *testing.T, event *EventNotification[T], client *Client, wantID, wantType string, wantCreatedAt time.Time) {
	t.Helper()
