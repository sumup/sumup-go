...
// nonNullSchema unwraps a nullable union only when it has a single value type.
// Keep references intact so callers retain the named Go type.
func nonNullSchema(schema *base.SchemaProxy) *base.SchemaProxy {
	...
	var value *base.SchemaProxy
	hasNull := false
	for _, option := range options {
		if option == nil || option.Schema() == nil {
			return schema
		}
		if slices.Equal(option.Schema().Type, []string{"null"}) {
			hasNull = true
			continue
		}
		if value != nil {
	...
	if hasNull && value != nil {
		return nonNullSchema(value)
	}
	return schema
}

func dereferenceSchema(ref *base.SchemaProxy) *base.SchemaProxy {
	ref = nonNullSchema(ref)
	if ref == nil {
		return nil
	}
